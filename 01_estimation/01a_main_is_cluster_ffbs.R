...
sl_best$qf <- as.numeric(as.character(sl_best$qf))
sl_best2 <- sl_best
sl_best2$qf <- 1

sl_best <- rbind(sl_best2,sl_best); rm(sl_best2)
...
run.spec <- sl_best[run,]

# set grid for the forecast exercise
run.h <- 8
run.model <- "mfreg"
run.sv <- tolower(substr(run.spec$model,8,12)) == "sv"
run.cc <- run.spec$country
run.qf <- as.numeric(as.character(run.spec$qf))

run.csr <- unlist(strsplit(as.character(run.spec$csr),":"))[2]
run.csr <- ifelse(run.csr == "no", "none", run.csr)

run.me <- unlist(strsplit(as.character(run.spec$me),":"))[2]
...
data_regraw <- window(gva_ls[[as.character(run.cc)]]$regraw,start=start, end = end.a) # replace with true GVA level to compute shares (at the moment: index!!!)
...
omega <- data_regraw/apply(data_regraw,1,sum)
...
# consider release calendar (exclude final two yearly observations -- only in OOS)
# don't exclude this for full in-sample results!
...
if(run.model == "mfreg"){
  ...
  sl_quants <- c(0.05,0.1,0.16,0.5,0.84,0.9,0.95)
  Z_ann_post <- apply(Z_store, c(2,3), quantile, probs = sl_quants)
  Z_lat_post <- apply(Zlat_store, c(2,3), quantile, probs = sl_quants)
  
  Y_true <- matrix(data_reg, ncol = ncol(data_reg), nrow = nrow(data_reg))
  colnames(Y_true) <- colnames(data_reg)
  rownames(Y_true) <- as.character(as.Date.ts(data_reg) %m+% months(9))
  
  out <- list("Z_ann" = Z_ann_post,
              "Z_lat" = Z_lat_post,
              "Y" = Y_true)
}
